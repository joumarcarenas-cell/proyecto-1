...
void MainMenuScene::Update(float dt) {
        ...
        IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
    {
        SceneManager::Get().ChangeScene(
            std::make_unique<CharacterSelectScene>(g_reaper, g_ropera, g_activePlayer));
        return;
    }
...
void MainMenuScene::Draw() {
    ...
    DrawText(title,
             sw / 2 - MeasureText(title, tsz) / 2,
             (int)titleY, tsz, GOLD);
    DrawText(subtitle,
             sw / 2 - MeasureText(subtitle, subSz) / 2,
    ...
    // Botones
    Vector2 mouse = GetMousePosition();
    DrawButton(m_btnPlay,     "JUGAR",    {100, 0, 160, 255}, CheckCollisionPointRec(mouse, m_btnPlay));
    DrawButton(m_btnSettings, "AJUSTES",  {40,  40, 80, 255}, CheckCollisionPointRec(mouse, m_btnSettings));
    DrawButton(m_btnQuit,     "SALIR",    {90,  20, 20, 255}, CheckCollisionPointRec(mouse, m_btnQuit));
...
void MainMenuScene::DrawButton(Rectangle btn, const char* label,
    ...
    // Sombra del botón
    DrawRectangle((int)btn.x + 4, (int)btn.y + 4, (int)btn.width, (int)btn.height, Fade(BLACK, 0.3f));
    
    // Efecto de brillo exterior en hover
    if (hovered) {
        DrawRectangleLinesEx({ btn.x - 5, btn.y - 5, btn.width + 10, btn.height + 10 }, 1.5f, Fade(bg, 0.4f));
    }
