  ...
  if (!m_boss.isDead) {
    ...
    if (!m_activePlayer->IsImmune() && dist < minDist && dist > 0) {
      Vector2 push = Vector2Normalize(
          Vector2Subtract(m_activePlayer->position, m_boss.position));
      float ov = minDist - dist;
      m_activePlayer->position =
          Vector2Add(m_activePlayer->position, Vector2Scale(push, ov * 0.5f));
      m_boss.position =
          Vector2Subtract(m_boss.position, Vector2Scale(push, ov * 0.5f));
    }
  }
...
void GameplayScene::UpdateBleedDoT(float dt) {
  if (!m_boss.isBleeding || m_boss.isDead || isTimeStopped)
    return;
  ...
  if (m_boss.bleedTickTimer <= 0) {
    ...
    m_damageTexts.push_back({m_boss.position,
...
void GameplayScene::UpdateRocks(float dt) {
  for (int i = 0; i < m_boss.rocksSpawned; i++) {
    ...
    if (m_boss.rocks[i].fallTimer <= 0) {
      ...
      Vector2 diff =
          Vector2Subtract(m_activePlayer->position, m_boss.rocks[i].position);
...
void GameplayScene::UpdateCollisions() {
  if (hitstopTimer > 0 || m_boss.isDead)
    return;
  ...
  // Burst direccional: chispas doradas + fragmentos purpura siguiendo el tajo
  Graphics::SpawnImpactBurst(
      m_boss.position, m_activePlayer->facing,
      {255, 220, 80, 255}, // chispas doradas
      {200, 0, 255, 255},  // fragmentos purpura (color de daño)
      12, 6);
}

// ─── UpdateDeathCheck ─────────────────────────────────────────────────────
void GameplayScene::UpdateDeathCheck() {
  // Si el jugador ha muerto o el boss ha muerto, mostramos el cursor para los
  // menús
  if (m_activePlayer->hp <= 0.0f || m_boss.hp <= 0.0f) {
    ShowCursor();
  }
}
...
void GameplayScene::DrawWorld() {
  ...
  Graphics::RenderManager::GetInstance().Submit(m_boss.GetZDepth(),
                                                [this]() { m_boss.Draw(); });
...
void GameplayScene::DrawHUD() {
  bool showHUD =
      (m_activePlayer->hp > 0 && !m_boss.isDead) || (m_activePlayer->hp > 0);
  ...
  if (showHUD) {
    ...
    const char *bName = (m_boss.hp <= m_boss.maxHp * 0.5f)
                            ? "BOSS ENFURECIDO"
                            : "GUARDIAN DE LA ARENA";
    DrawText(bName, (int)(bx + bw) - MeasureText(bName, 18), (int)(by + bh + 5),
             18, GOLD);
  ...
  if (m_activePlayer->hp <= 0.0f) {
    ...
    auto DrawDeadBtn = [&](Rectangle btn, const char *label, Color col) {
      ...
      int fs = 22;
      DrawText(label, (int)(btn.x + btn.width / 2 - MeasureText(label, fs) / 2),
               (int)(btn.y + btn.height / 2 - fs / 2), fs, WHITE);
  ...
  if (m_boss.hp <= 0.0f) {
    ...
    DrawCircleGradient(GetScreenWidth() / 2, GetScreenHeight() / 2, 400,
                       Fade(GOLD, 0.15f), {0, 0, 0, 0});
    ...
    DrawText(vicSub, GetScreenWidth() / 2 - MeasureText(vicSub, 20) / 2, 280,
             20, LIGHTGRAY);
    ...
    DrawText("MENU PRINCIPAL",
             (int)(btnMenu.x + btnMenu.width / 2 -
                   MeasureText("MENU PRINCIPAL", fs) / 2),
             (int)(btnMenu.y + btnMenu.height / 2 - fs / 2), fs, BLACK);
