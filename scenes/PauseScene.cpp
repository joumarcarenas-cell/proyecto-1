...
void PauseScene::Update(float /*dt*/) {
  ...
  if (IsKeyPressed(KEY_K) || IsKeyPressed(KEY_ESCAPE) ||
    ...
    SceneManager::Get()
        .PopOverlay(); // Cierra el overlay; GameplayScene reanuda
    return;
  }
...
void PauseScene::Draw() {
  ...
  DrawRectangleLinesEx({px - 2, py - 2, pw + 4, ph + 4}, 1.0f,
                       Fade(GOLD, 0.3f));
  ...
  // Línea decorativa
  DrawLineEx({px + 40, py + 95}, {px + pw - 40, py + 95}, 2.0f,
             Fade(GOLD, 0.4f));
  ...
  auto DrawBtn = [&](Rectangle btn, const char *label, Color col) {
    ...
    if (hov) {
      DrawRectangleLinesEx(
          {btn.x - 4, btn.y - 4, btn.width + 8, btn.height + 8}, 1.5f,
          Fade(bg, 0.4f));
    }
    ...
    int fs = 24;
    DrawText(
        label, (int)(btn.x + btn.width * 0.5f) - MeasureText(label, fs) / 2 + 1,
        (int)(btn.y + btn.height * 0.5f) - fs / 2 + 1, fs, Fade(BLACK, 0.6f));
    DrawText(label,
             (int)(btn.x + btn.width * 0.5f) - MeasureText(label, fs) / 2,
             (int)(btn.y + btn.height * 0.5f) - fs / 2, fs, WHITE);
  };
  ...
  DrawText("PULSA K O ESC PARA VOLVER AL COMBATE",
           GetScreenWidth() / 2 -
               MeasureText("PULSA K O ESC PARA VOLVER AL COMBATE", 16) / 2,
